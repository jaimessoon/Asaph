...
# Check for Google Login (Streamlit 1.42+ syntax)
...
def scrape_ug_details(url):
    ...
    # Clean up formatting to ChordPro style
...
with st.sidebar:
    ...
    try:
        sources_df = conn.read(spreadsheet=SHEET_URL, worksheet="Sources", ttl=600)
    except Exception:
        sources_df = pd.DataFrame(columns=["User_Email", "Site_Name", "Base_URL"])

    user_sources = sources_df[sources_df['User_Email'] == st.user.email]
    ...
    with st.expander("➕ Add Custom Source"):
        ...
        if st.button("Register Source"):
            new_s_row = pd.DataFrame([{"User_Email": st.user.email, "Site_Name": new_site, "Base_URL": new_base}])
...
with tab2:
    ...
    try:
        ...
        my_songs = lib_df[lib_df['User_Email'] == st.user.email]
